...
DEFAULT_DATASET = APP_DIR / "data" / "Smooth_Hardbound_DGA_HI_Dataset.xlsx"
...
OTI_WEIGHT = 0.075
WTI_WEIGHT = 0.075
DGA_WEIGHT = 1.0 - BDV_WEIGHT - OTI_WEIGHT - WTI_WEIGHT
...
def expand_with_operating_conditions(df: pd.DataFrame, hi_points: pd.Series, seed: int = 42) -> pd.DataFrame:
    ...
    for source_index, record in enumerate(df.to_dict("records")):
        base_hi = float(hi_points.iloc[source_index])
        scores = gas_scores(record)
        dga_critical = label_rule_features(scores)["label_rule_any_gas_critical"] == 1.0
        dga_target = 0.0 if dga_critical else base_hi

        for profile in profiles:
            row = dict(record)
            if profile == "normal":
                row["BDV"] = float(rng.uniform(58, 75))
                row["OTI"] = float(rng.uniform(58, 76))
                row["WTI"] = float(row["OTI"] + rng.uniform(5, 14))
            elif profile == "low_bdv":
                row["BDV"] = float(rng.uniform(40, 50))
                row["OTI"] = float(rng.uniform(65, 82))
                row["WTI"] = float(row["OTI"] + rng.uniform(7, 13))
            elif profile == "thermal_watch":
                row["BDV"] = float(rng.uniform(48, 63))
                row["OTI"] = float(rng.uniform(82, 89.8))
                row["WTI"] = float(rng.uniform(88, 94.8))
            elif profile == "near_limit":
                row["BDV"] = float(rng.uniform(40, 45))
                row["OTI"] = float(rng.uniform(86, 90))
                row["WTI"] = float(rng.uniform(91, 95))
            else:
                cap_case = int(rng.integers(0, 3))
                row["BDV"] = float(rng.uniform(30, 39.5) if cap_case == 0 else rng.uniform(45, 62))
                row["OTI"] = float(rng.uniform(90.2, 99) if cap_case == 1 else rng.uniform(70, 88))
                row["WTI"] = float(rng.uniform(95.2, 108) if cap_case == 2 else rng.uniform(80, 94))

            op_scores = operating_scores(row)
            op_flags = operating_limit_features(row)
            operating_cap = any(
                op_flags[name] == 1.0
                for name in ("BDV_below_40_flag", "OTI_above_90_flag", "WTI_above_95_flag")
            )
            if dga_critical or operating_cap:
                final_hi = 0.0
            else:
                final_hi = (
                    DGA_WEIGHT * dga_target
                    + BDV_WEIGHT * op_scores["BDV_score"]
                    + OTI_WEIGHT * op_scores["OTI_score"]
                    + WTI_WEIGHT * op_scores["WTI_score"]
                )
            row["Operating_Profile"] = profile
            row["DGA_HI_training_component"] = dga_target
            row["HI_before_label_rule"] = base_hi
            row["HI_training_target"] = float(np.clip(final_hi, 0.0, 100.0))
            row["operating_relation_used_for_training"] = 1
            rows.append(row)
...
def load_training_data(dataset_path: Path) -> tuple[np.ndarray, np.ndarray, pd.DataFrame]:
    ...
    df = expand_with_operating_conditions(df, hi_points)
    critical_flags = []
...
def build_augmented_dataset(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in df.to_dict("records"):
        ...
        for key, value in scores.items():
            record[key] = value
        for key, value in operating_scores(row).items():
            record[key] = value
        for key, value in operating_limit_features(row).items():
            record[key] = value
        for gas, condition in conditions.items():
            record[f"{gas}_condition_from_threshold_table"] = condition
            if condition != "Healthy":
                record[f"{gas}_reason_from_threshold_table"] = gas_reason(gas)
        for key, value in summary.items():
            record[key] = value
        for key, value in label_features.items():
            record[key] = value
        record["Label_Rule_Derived"] = label_rule_name(scores)
    ...
    augmented["BDV_OTI_WTI_relation_used_for_training"] = 1
    augmented["BDV_OTI_WTI_relation"] = "Final HI = 70% DGA + 15% BDV + 7.5% OTI + 7.5% WTI below hard caps; hard caps force HI 0."
    return augmented
...
def main() -> None:
    ...
    metadata = {
        ...
        "target_rule": "HI_training_target = 0 when any gas crosses Poor Max, BDV < 40, OTI > 90, or WTI > 95; otherwise weighted relation 70% DGA + 15% BDV + 7.5% OTI + 7.5% WTI",
        "operating_relation": {
            "DGA_weight": DGA_WEIGHT,
            "BDV_weight": BDV_WEIGHT,
            "OTI_weight": OTI_WEIGHT,
            "WTI_weight": WTI_WEIGHT,
            "BDV_hard_cap": "BDV < 40 => HI 0",
            "OTI_hard_cap": "OTI > 90 => HI 0",
            "WTI_hard_cap": "WTI > 95 => HI 0",
        },
